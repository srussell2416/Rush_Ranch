...
data %>%
  ...
  geom_point(alpha = 0.6, size = 0.7 , aes(color = Year)) +
  facet_wrap(~Var, scales='free_x') +
  geom_smooth(method = "loess", size = 0.5) +
  labs(y = "GPP (g C m-2 day-2)", x = NULL) +
  theme_bw()

```

## Growing Season Interannual Variability in Environmental Drivers

```{r, echo=FALSE, warning=FALSE, fig.height = 9, fig.width = 8}
p2 <- data %>%
  left_join(data2[, c('date', 'Sal', 'pH', 'Depth','RH', 'SWC', 'SunCount')], by = 'date') %>%
  filter(Month > 6 & Month < 9,
         SunCount == 30) %>%
  mutate(Tair = as.numeric(as.character(Tair)),
         Depth = as.numeric(as.character(Depth)),
         pH = as.numeric(as.character(pH)),
         VPD_f = as.numeric(as.character(VPD_f)),
         RH = as.numeric(as.character(RH)),
  ...
  summarize(Sal = mean(Sal), 
            ...
            RH = mean(RH),
            SWC = mean(SWC)) %>%
  pivot_longer(cols = c("Sal", "Tair", "pH", "Depth", "VPD_f", "RH", 'SWC'), 
               names_to = "Var", 
               values_to = "Val") %>%
  ggplot(aes(x = DOY2, y = Val)) +
  geom_line(aes(color = Year)) +
  facet_wrap(~Var, scales='free_y', nrow = 7) +
  labs(y = NULL, x = "Days Since April 1st") +
  theme_bw() +
...
p1 <- data %>%
  ...
  pivot_longer(cols = c("Sal", "Tair", "pH", "Depth", "VPD_f", "RH", 'SWC'), 
               names_to = "Var", 
               values_to = "Val") %>%
  ggplot(aes(x = Year, y = Val, col = Year)) +
  facet_wrap(~Var, scales='free_y', nrow = 7) +
  geom_boxplot() +
  labs(y = "Value", x = "Growing Season Year") +
  theme_bw() +
